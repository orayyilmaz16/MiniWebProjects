...
export default function Login() {
  ...
  const { login } = useAuth();
  const [email, setEmail] = useState("admin@authent.dev");
      ...
      <div className="mx-auto max-w-lg">
        <div className="rounded-xl2 border border-zinc-200 bg-white p-6 shadow-soft dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-300">
            Demo admin: admin@authent.dev / Admin123! — demo user: user@authent.dev / User123!
          </p>
          ...
          <p className="mt-4 text-sm text-zinc-600 dark:text-zinc-300">
            Hesabın yok mu? <Link className="font-semibold underline" to="/register">Kayıt ol</Link>
          </p>
        </div>
      </div>
    </AnimatedSection>
